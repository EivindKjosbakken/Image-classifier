...
def compute_loss_and_accuracy(
    ...
    with torch.no_grad():
        for (X_batch, Y_batch) in dataloader:
            ...
            output_probs = output_probs.argmax(axis=1).squeeze()
            Y_batch = Y_batch.squeeze()

            correctPredictions += (output_probs == Y_batch).sum() #sum up everywhere the model predicted correctly
            numberOfSteps+=1
            numberOfImgs += output_probs.shape[0]

            # Compute Loss and Accuracy

    averageLoss = loss/numberOfSteps
    accuracy = correctPredictions/numberOfImgs
...
class Trainer:
    ...
    def __init__(self,
        ...
        # Define our optimizer, changed depending on task, Adam for one model in task 3,
        #  SGD with momentum/weight decay for the other model in task 3, 
        # and the baseline SGD for task 2
        
        #for task 3:
        #self.optimizer = torch.optim.Adam(self.model.parameters(), #changed optimizer to Adam
        #                                 self.learning_rate)
        
        #self.optimizer = torch.optim.SGD(self.model.parameters(), 
        #        learning_rate=self.learning_rate, momentum=1e-4, weight_decay=1e-4)
            ...
            for X_batch, Y_batch in self.dataloader_train:
                ...
                if should_validate_model():
                    ...
                    if self.should_early_stop():
                        ...
                        self.printTestAcc()
                        return
        #also show test acc if it did not early stop
        self.printTestAcc()


    def printTestAcc(self):
        #for printing test loss and accuracy
        test_loss, test_acc = compute_loss_and_accuracy(
            self.dataloader_test, self.model, self.loss_criterion
        )    
        print("test acc is: ", test_acc, " loss is: ", test_loss) 
